 ...
 * <p>
 * Elle gère l'affichage du texte, des images et la saisie des commandes par
 * l'utilisateur via un champ de texte. Cette classe est responsable de
 * l'interaction avec la logique du jeu représentée par la classe {@link Jeu}.
 * <p>
 * Les principales fonctionnalités incluent :
 * <ul>
 * <li>Afficher des messages dans un JTextArea</li>
 * <li>Afficher des images associées aux zones du jeu</li>
 * <li>Recevoir et transmettre les commandes utilisateur à la classe
 * {@link Jeu}</li>
...
public class GUI implements ActionListener {
	...
	/** Affiche une image depuis le dossier resources en la redimensionnant */
	public void afficheImage(String nomImage) {
		SwingUtilities.invokeLater(() -> {
			URL imageURL = getClass().getClassLoader().getResource(NomRepertoireImages + nomImage);
			if (imageURL != null) {
				// Загружаем оригинальную картинку
				javax.swing.ImageIcon iconOriginal = new javax.swing.ImageIcon(imageURL);
				java.awt.Image img = iconOriginal.getImage();

				// Сжимаем её до адекватного размера (например, 800x450 пикселей)
				// SCALE_SMOOTH делает сжатие качественным, без пикселей
				java.awt.Image imgRedimensionnee = img.getScaledInstance(800, 450, java.awt.Image.SCALE_SMOOTH);

				// Ставим сжатую картинку
				image.setIcon(new javax.swing.ImageIcon(imgRedimensionnee));
				fenetre.pack();
			} else {
